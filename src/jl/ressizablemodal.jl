...
export ressizablemodal

"""
    ressizablemodal(;kwargs...)
    ressizablemodal(children::Any;kwargs...)
    ressizablemodal(children_maker::Function;kwargs...)


A RessizableModal component.
...
- `x` (Real; optional): Initial X position in pixels from the left edge.
- `y` (Real; optional): Initial Y position in pixels from the top edge.
- `zIndex` (Real; optional): Z-index for the modal (backdrop will be zIndex - 1).
"""
function ressizablemodal(; kwargs...)
        available_props = Symbol[:children, :id, :animation, :backdrop, :backdropClosable, :bodyClassName, :bodyStyle, :className, :customResizeIcon, :dragBounds, :dragHandle, :draggable, :escapeClosable, :headerClassName, :headerStyle, :height, :isOpen, :maxHeight, :maxWidth, :minHeight, :minWidth, :modal, :onClose, :onMove, :onResize, :resizable, :resizeCorner, :resizeHandleClassName, :resizeHandleStyle, :showResizeIcon, :style, :title, :width, :x, :y, :zIndex]
        wild_props = Symbol[]
        return Component("ressizablemodal", "RessizableModal", "dash_panels", available_props, wild_props; kwargs...)
end

ressizablemodal(children::Any; kwargs...) = ressizablemodal(;kwargs..., children = children)
ressizablemodal(children_maker::Function; kwargs...) = ressizablemodal(children_maker(); kwargs...)
